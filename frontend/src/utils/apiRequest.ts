...
export const apiRequest = async (params = {}) => {
  const {
    ...
    onError = null,
    showMessages = true,
    // Nuevo: mostrar loader en pantalla completa durante la petición
    loader = false,
  ...
  try {
    if (loader) {
      loadingOverlayStore.showOverlay(loaderFullscreen, loaderText);
    }
    if (!url) {
      console.log("Falta URL en la solicitud API");
      if (loader) loadingOverlayStore.hideOverlay();
      return;
    }
    ...
    if (dataResponse.result || dataResponse.success) {
      if (showMessages) {
        const message =
